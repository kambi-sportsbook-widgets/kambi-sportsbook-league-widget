   ...
   var LeagueTable = CoreLibrary.Component.subclass({
      ...
      init: function init() {
         ...
         if (CoreLibrary.pageInfo.leaguePaths != null && CoreLibrary.pageInfo.leaguePaths.length === 1) {
            CoreLibrary.statisticsModule.getStatistics('leaguetable', CoreLibrary.pageInfo.leaguePaths[0]).then(function (data) {
               var rows = [],
               ...
               _this.scope.leagueTableRows = rows;
               _this.scope.args.updatedTime = date;

               // Calculate the height based on the rows plus the header and footer divs
               var rowHeight = 45;
               var calculatedHeight = _this.scope.leagueTableRows.length * rowHeight + 84;
      ...
      getColumnLabels: function getColumnLabels() {
